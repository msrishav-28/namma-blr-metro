...
const isValidCoordinatePair = (station: StationInfo | undefined): station is StationWithCoordinates =>
  ...
  station.Latitude >= 12.5 &&
  station.Latitude <= 13.5 &&
  station.Longitude >= 77.0 &&
  station.Longitude <= 78.0;
...
const calculateEstimatedMinutes = (routePath: string[], interchangeCount: number) => {
  const totalMinutes = routePath.slice(0, -1).reduce((minutes) => {
    return minutes + 2.2;
  }, interchangeCount * 5);
...
const getRouteTimeLimitMinutes = (distanceKm: number) => {
...
const buildRoutePlan = (
  ...
  return {
    ...
    route: {
      ...
      timeLimitMinutes: getRouteTimeLimitMinutes(distanceKm),
