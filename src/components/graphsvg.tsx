...
const ModelViewer = React.forwardRef<HTMLElement, any>((props, ref) => 
  React.createElement('model-viewer', { ref, ...props })
);
...
const makeShortsScriptSegments = (
  ...
  const routeIntro = {
    en: `We are travelling from ${fromName} to ${toName} by Namma Metro.`,
    hi: `हम ನಮ್ಮ ಮೆಟ್ರೋ से ${fromName} से ${toName} तक यात्रा कर रहे हैं.`,
    mr: `आपण ನಮ್ಮ ಮೆಟ್ರೋने ${fromName} ते ${toName} प्रवास करत आहोत.`,
    bn: `আমরা ನಮ್ಮ ಮೆಟ್ರೋতে ${fromName} থেকে ${toName} যাচ্ছি.`,
    pa: `ਅਸੀਂ ನಮ್ಮ ಮೆಟ್ರೋ ਰਾਹੀਂ ${fromName} ਤੋਂ ${toName} ਜਾ ਰਹੇ ਹਾਂ.`,
  }[language];
  ...
  const scripts = {
    en: [
      ...
      { text: 'Plan your Namma Metro trip at namma blr metro dot vercel dot app.', stationId: route.to },
    ],
    hi: [
      { text: routeIntro, stationId: route.from },
      { text: directionSentence, stationId: firstDirection?.stationId || route.from },
      ...
      { text: 'अपनी ನಮ್ಮ ಮೆಟ್ರೋ यात्रा namma blr metro dot vercel dot app पर प्लान करें.', stationId: route.to },
    ],
    mr: [
      { text: routeIntro, stationId: route.from },
      { text: directionSentence, stationId: firstDirection?.stationId || route.from },
      ...
      { text: 'तुमचा ನಮ್ಮ ಮೆಟ್ರೋ प्रवास namma blr metro dot vercel dot app वर प्लान करा.', stationId: route.to },
    ],
    bn: [
      { text: routeIntro, stationId: route.from },
      { text: directionSentence, stationId: firstDirection?.stationId || route.from },
      ...
      { text: 'আপনার ನಮ್ಮ ಮೆಟ್ರೋ যাত্রা namma blr metro dot vercel dot app এ প্ল্যান করুন.', stationId: route.to },
    ],
    pa: [
      { text: routeIntro, stationId: route.from },
      { text: directionSentence, stationId: firstDirection?.stationId || route.from },
      ...
      { text: 'ਆਪਣੀ ನಮ್ಮ ಮೆಟ್ರೋ ਯਾਤਰਾ namma blr metro dot vercel dot app ਤੇ ਪਲਾਨ ਕਰੋ.', stationId: route.to },
    ],
  };
...
function SvgComponent({
  ...
  const setCameraForProgress = React.useCallback((progress: number, cameraScale = routeCameraScale) => {
    ...
    // Apply 2D translation only to the SVG group to keep the 3D model upright
    train.setAttribute('transform', `translate(${point.x} ${point.y})`);
    
    // Apply 3D yaw rotation directly to the model-viewer
    if (modelViewerRef.current) {
      // SVG Y goes down (clockwise), 3D Yaw typically uses right-hand rule (counter-clockwise)
      // If the train drives backwards or sideways, tweak this offset (e.g., -angle + 90)
      modelViewerRef.current.setAttribute('orientation', `0deg ${-angle}deg 0deg`);
    }
  ...
  const downloadFullMapSvg = React.useCallback(async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = 'namma-metro-full-map.svg';
      link.click();
  ...
  const downloadRouteVideo = React.useCallback(async () => {
    ...
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `namma-metro-route-${animationMode}.webm`;
      link.click();
      window.setTimeout(() => URL.revokeObjectURL(url), 0);
    } catch (error) {
      console.error(error);
  ...
  const drawShortsFrame = React.useCallback(async (
    ...
    context.fillText('NAMMA METRO ROUTE', 96, 154);
  ...
  const downloadYoutubeShortsVideo = React.useCallback(async () => {
    ...
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `namma-metro-shorts-${route.from}-to-${route.to}.mp4`.toLowerCase();
      link.click();
      window.setTimeout(() => URL.revokeObjectURL(url), 0);
    } catch (error) {
      console.error(error);
              ...
              <>
                ...
                >
                  <foreignObject x={-40} y={-40} width={80} height={80}>
                    <ModelViewer
                      ref={modelViewerRef}
                      src="https://github.com/msrishav-28/namma-blr-metro/releases/download/v1.0.0/blr_metro_train.glb"
