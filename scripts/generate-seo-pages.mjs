...
const baseUrl = (process.env.SITE_URL || 'https://namma-blr-metro.vercel.app').replace(/\/$/, '');
...
const renderHtml = ({ title, description, keywords, canonicalPath, body, schema, hydrationData }) => {
  ...
  html = html.replace(
    '</head>',
    `    ${hydrationData ? `<script>window.__NAMMA_METRO_ROUTE__=${JSON.stringify(hydrationData)};</script>\n    ` : ''}<script type="application/ld+json">${JSON.stringify(schema)}</script>\n  </head>`
  );
...
const stationPage = (station) => {
  ...
  const title = `${station.text} Metro Station | Namma Metro`;
  const description = `Check ${station.text} metro station gates, platforms, facilities, nearby places, parking, route links, fare planning, and Namma Metro travel details.`;
  const keywords = `${station.text} metro station, ${station.text} gates, ${station.text} facilities, ${station.text} Namma Metro route, metro from ${station.text}, Namma Metro fare from ${station.text}`;
  ...
  const body = `
    <main class="seo-prerender">
      ...
      <img src="${showcaseImagePath}" alt="Namma Metro route planner map for ${escapeHtml(station.text)} station" width="1200" height="630" loading="lazy" />
      ...
      <p>
        ${escapeHtml(station.description || `${station.text} Metro Station is part of the Namma Metro network and can be used as a starting point or destination in the route planner.`)}
      </p>
      ...
      <p>
        ...
        These links help you move through the Namma Metro route network and discover shorter station-to-station journeys.
        For a broader starting point, open the <a href="/">Namma Metro Route Planner</a> homepage or browse key stations
        like <a href="/stations/majestic/">Majestic</a>, <a href="/stations/indiranagar/">Indiranagar</a>, and
        <a href="/stations/yeshwanthpur/">Yeshwanthpur</a>.
      </p>
      <p>
        ...
        the same station often and for visitors who are checking a Namma Metro journey for the first time.
      </p>
      <p>
        ...
        visit the <a href="https://www.english.bmrc.co.in/" target="_blank" rel="noopener noreferrer">Bangalore Metro Rail Corporation Limited</a>.
      </p>
    </main>`;
  const schema = {
...
const routePage = (route) => {
  ...
  const description = `Check the ${route.fromName} to ${route.toName} Namma Metro route map with estimated fare Rs ${route.fare}, travel time ${route.estimatedMinutes} minutes, ${route.stops} stops, distance about ${route.distanceKm} km, and interchange guidance.${interchangeText}`;
  const keywords = `${route.fromName} to ${route.toName} metro route, ${route.fromName} to ${route.toName} metro route map, ${route.fromName} to ${route.toName} metro fare, ${route.fromName} to ${route.toName} metro time, Namma Metro ${route.fromName} ${route.toName}, ${route.fromName} to ${route.toName} stops`;
  ...
  const body = `
    <main class="seo-prerender">
      ...
      <img src="${showcaseImagePath}" alt="Namma Metro route planner for ${escapeHtml(route.fromName)} to ${escapeHtml(route.toName)}" width="1200" height="630" loading="lazy" />
      <p>${escapeHtml(description)}</p>
      <dl>
      ...
      <p>
        This Namma Metro route starts at <a href="${stationPathname(route.from)}">${escapeHtml(route.fromName)}</a> and
      ...
      <p>
        ...
        line, and operational notices, so verify time-critical trips with official Namma Metro sources.
      </p>
      <p>
  ...
  const schema = {
    '@context': 'https://schema.org',
    '@type': 'Trip',
    name: `${route.fromName} to ${route.toName} Namma Metro route`,
    url: `${baseUrl}${pathname}`,
    departureStation: {
      '@type': 'TrainStation',
      name: route.fromName,
...
const routeIndexPage = () => {
  ...
  const title = 'Namma Metro Routes | Route Map, Fare, Time & Stops';
  const description = 'Browse Namma Metro station-to-station route pages with route map links, estimated fare, travel time, stop count, distance, and interchange guidance.';
  const keywords = 'Namma Metro routes, Namma Metro route map, Namma Metro fare, Namma Metro travel time, Namma Metro station to station routes, Namma Metro stops';
  ...
  const body = `
    <main class="seo-prerender">
      <h1>Namma Metro Routes</h1>
      <img src="${showcaseImagePath}" alt="Namma Metro route map and station-to-station planner" width="1200" height="630" loading="lazy" />
      <p>${escapeHtml(description)}</p>
      <h2>Popular Namma Metro route pages</h2>
      <ul>
      ...
      <p>
        ...
        number of stops, distance, and interchange stations. Open the <a href="/">Namma Metro Route Planner</a> to search
  ...
  const schema = {
    '@context': 'https://schema.org',
    '@type': 'CollectionPage',
    name: 'Namma Metro Routes',
    url: `${baseUrl}${pathname}`,
    description,
    isPartOf: {
      '@type': 'WebSite',
      name: 'Namma Metro Route Planner',
      url: `${baseUrl}/`,
    },
  };
...
const openSourceAlternativePage = (alternative) => {
  ...
  const title = `${alternative.headline} | Namma Metro Route Planner`;
    ...
    keywords: 'Namma Metro Route Planner privacy policy, namma-blr-metro.vercel.app privacy, Namma Metro app privacy',
    heading: 'Privacy Policy',
    body: `
      <p>Last updated: June 02, 2026</p>
      <p>This Privacy Policy describes the collection, use, and disclosure of information when you use Namma Metro Route Planner.</p>
      ...
      <h2>Contact</h2>
      <p>For questions about this Privacy Policy, contact <a href="mailto:msrishav28@gmail.com">msrishav28@gmail.com</a>.</p>
    `,
  },
  {
    pathname: '/terms-and-conditions/',
    title: 'Terms and Conditions | Namma Metro Route Planner',
    description: 'Read the Terms and Conditions for Namma Metro Route Planner, including acceptable use, accuracy, limitations, and contact information.',
    keywords: 'Namma Metro Route Planner terms, namma-blr-metro.vercel.app terms and conditions, Namma Metro route planner disclaimer',
    heading: 'Terms and Conditions',
    body: `
      <p>Last updated: June 02, 2026</p>
      <p>By accessing or using Namma Metro Route Planner, you agree to these Terms and Conditions.</p>
      <h2>Service</h2>
      <p>Namma Metro Route Planner is an independent route planning tool for general guidance and is not affiliated with, endorsed by, or operated by Bangalore Metro Rail Corporation Limited.</p>
      <h2>Accuracy</h2>
      ...
      <h2>Contact</h2>
      <p>For questions about these Terms and Conditions, contact <a href="mailto:msrishav28@gmail.com">msrishav28@gmail.com</a>.</p>
    `,
  },
];
...
const legalPage = (page) => {
  const body = `
    <main class="seo-prerender">
      ...
      ${page.body}
      <p><a href="/">Open Namma Metro Route Planner</a></p>
    </main>`;
  const schema = {
    '@context': 'https://schema.org',
    '@type': 'WebPage',
